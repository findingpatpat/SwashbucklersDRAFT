...
function render(){
  ...
  results.innerHTML=state.finishOrder.length
    ? `<h3>FINAL DRAFT ORDER</h3>`+state.finishOrder.map(r=>`<div class="result"><strong>#${r.place}</strong><span>${escapeHtml(r.name)}</span><span>${formatRaceTime(r.finishTimeMs)}</span></div>`).join("")
    : "";
}
